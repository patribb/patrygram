...
const Post = ({ userName, photoURL, caption, imageURL, postID }) => {
    ...
    const [likeState, setLikeState] = useState({like: likesOnPost?.likes.length > 0 ? likesOnPost?.likes.length : 0, likeActive: false});
    const [commentsOnPost, setCommentsOnPost] = useState([]);
    const [commentState, setCommentState] = useState({ comments: commentsOnPost?.length > 0 ? commentsOnPost?.length : 0});
  ...
  return (
    <Container>
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle style={{ textAlign: 'center'}}>Comentarios</DialogTitle>
        <DialogContent>
          <AllCommentContainer>
     ...
     <CTAButtons>
          {likeState.likeActive ? (
            <img src="./heart1.png" alt="" onClick={handleLike} />
          ) : (
            <img src="./heart.png" alt="" onClick={handleLike} />
          )}
          <img src="./chat.png" alt="" onClick={() => setOpenDialog(true)} />
        </CTAButtons>
        <LikeCount>
        ...
        <PostDescription moreButton={moreButton}>
            ...
            <div className="description-buttons">
                <p onClick={() => setOpenDialog(true)}>Ver comentarios</p>
                <p onClick={()=>setMoreButton(!moreButton)}>{moreButton ? 'Más...' : 'Menos..'}</p>
            </div>
        </PostDescription>
        <CommentInput>
            <input type="text"  onChange={(e) => setCommentInput(e.target.value)}
            value={commentInput} placeholder='Añadir comentario...' />
